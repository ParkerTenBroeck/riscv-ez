...
impl eframe::App for MyApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::SidePanel::left("documents").show(ctx, |ui| {
            ScrollArea::both().scroll_bar_visibility(ScrollBarVisibility::VisibleWhenNeeded)
                .show(ui, |ui|{
                    for tab in [Tab::Terminal, Tab::Display, Tab::CPU, Tab::Log]{
                        ...
                        if ui.selectable_label(is_open, RichText::new(tab.str()).strong()).clicked() {
                    ...
                    let state = TreeView::new(ui.make_persistent_id("nya")).show_state(ui, &mut self.tree_view_state, |builder| {
                        let mut count = 0;
                        for path in context.project_paths(){
                            let now = path.str().matches("/").count().saturating_sub(path.is_file() as usize);
                            for _ in now..count{
                                builder.close_dir();
                            }
                            count = now;
                            let disp = path.str().split("/").last().unwrap_or("ERROR").to_owned();
                            if path.is_file(){
